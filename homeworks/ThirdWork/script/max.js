...
if (number >= 2 && number <= 9) {
  // числа для яких існує табличка множення
  for (let i = 1; i <= 10; i++) {
    // задаємо діапазон чисел на які множимо
    result = number * i;
...
if (day >= 1 && day <= 29) {
  ...
  // якщо 30 число то наступний день не +1 а перше число
  nextDay = 1;
}
if (day > 30) {
  // якщо день більше 30-го числа то додаєм місяць
  nextMonth += 1;
}
if (month >= 1 && month <= 11 && day === 30) {
  // задаємо що місяців 12 і якщо день 30 то збільшуємо дату місяця
  nextMonth += 1;
} else if (month === 12 && day === 30) {
  // якщо місяць 12 і день 30 то переходимо на перший місяць
  nextMonth = 1;
}
if (month === 12 && day === 30) {
  // якщо місяць 12 і день 30 то додаєємо рік
  nextYear += 1;
}
